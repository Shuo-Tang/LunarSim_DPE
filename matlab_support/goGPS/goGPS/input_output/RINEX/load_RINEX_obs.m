...
% % % % % codeC1 = zeros(1,1,nFiles);       % MODIFICATION FROM DANIEL
codeC1 = zeros(1,nEpochs,nFiles);
...
%         else
            pr1(:,k,f) = obs.C1;
% % % % %             codeC1(:,:,f) = 1;    % MODIFICATION FROM DANIEL
%             codeC1(:,k,f) = 1;
%         end
        pr2(:,k,f) = obs.P2;
        ph1(:,k,f) = obs.L1;
...
nEpochs = k - 1;
pseudorange1 = cell(nEpochs, 1);
for i = 1:nEpochs
    pseudorange1{i} = pr1(pr1(:,i) ~= 0, i);
end
